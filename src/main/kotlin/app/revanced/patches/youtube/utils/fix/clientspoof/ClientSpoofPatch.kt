...
import app.revanced.patches.shared.patch.transformation.IMethodCall
import app.revanced.patches.shared.patch.transformation.Instruction35cInfo
import app.revanced.patches.shared.patch.transformation.filterMapInstruction35c
...
object ClientSpoofPatch : AbstractTransformInstructionsPatch<Instruction35cInfo>() {
    ...
    override fun transform(mutableMethod: MutableMethod, entry: Instruction35cInfo) {
        ...
        mutableMethod.apply {
            ...
            val targetReference = getInstruction(instructionIndex + 2).getReference<MethodReference>()?.toString()

            // This can technically also match non-user agent string builder append methods,
            // but no such occurrences are present in the app.
            if (targetReference != USER_AGENT_STRING_BUILDER_APPEND_METHOD_REFERENCE) {
                return
            }
            ...
            replaceInstruction(
                instructionIndex + 1,
                "const-string v$targetRegister, \"${PACKAGE_NAME}\"",
            )
        }
    }
